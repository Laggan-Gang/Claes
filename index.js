...
const { createAudioPlayer, createAudioResource, joinVoiceChannel, AudioPlayerStatus, AudioPlayer } = require('@discordjs/voice');
...
function spelaungefärljudetavenbokstav(meddelande, bokstäver) {
	...
	for (let i = 0; i < bokstäver.length; i++) {
		...
		player.play(resurs)
		while ((player.state == AudioPlayerStatus.Playing || player.state == AudioPlayerStatus.Buffering)) //dumb baby jail
		{
		}
